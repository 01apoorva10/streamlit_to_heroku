...
def upload_multiple_file(uploaded_file):
...
def transform_uploaded_file(dir_path):
    ...
    for eachExcel_file in excel_file_list:
        workbook = load_workbook(dir_path+"/"+eachExcel_file,
...
# ------------------load workobook with xlwings module --------------------
        ...
        workbook = xw.Book(dir_path+"/"+eachExcel_file)
...
# first column of dataframe
        ...
        workbook.save(dir_path+"/"+eachExcel_file)
        workbook.close()
...
def main():
    ...
    if choice == 'single excel file trasformation':
        ...
        if datafile is not None:
            ...
            upload_single_file(datafile)
        directory_path = "singleFile"
        transform_uploaded_file(directory_path)
        download_single_file(directory_path)
        ...
        if datafile is not None:
            ...
            # print(datafile.name)
                upload_multiple_file(uploaded_file)
        directory_path = "multipleFiles"
        transform_uploaded_file(directory_path)
